...
export type Props = {
  stationName: string
  onNewChannel: React.MouseEventHandler
}

const StationControl: React.FC<Props> = ({ stationName, onNewChannel }) => {
  ...
  return (
    <Menu>
      <MenuHandler>
        <Button
          variant='text'
          className='flex items-center gap-3 justify-center font-normal capitalize tracking-normal w-full rounded-none'
        >
          <Typography variant='small'>{stationName}</Typography>
          <ChevronDownIcon
            strokeWidth={2.5}
            className={`h-3.5 w-3.5 transition-transform ${opened ? "rotate-180" : ""}`}
          />
        </Button>
      </MenuHandler>
      <MenuList placeholder={undefined}>
        ...
        <MenuItem className='flex items-center gap-2' placeholder={undefined}>
          ...
          <Typography variant='small'>Stats for nerds</Typography>
        </MenuItem>
      </MenuList>
    </Menu>
  )
